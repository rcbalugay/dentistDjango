...
class AppointmentFormTests(TestCase):
    ...
    def test_reject_closed_day(self):
        print("\n[TEST] closed clinic days are rejected")
    ...
    def test_reject_time_outside_clinic_hours(self):
        print("\n[TEST] off-hours appointments are rejected")
    ...
    def test_reject_time_not_on_the_hour(self):
        print("\n[TEST] non-hourly appointments are rejected")
    ...
    def test_allow_valid_future_open_slot(self):
        print("\n[TEST] valid future open slots are allowed")
    ...
    def test_reject_holiday_on_open_day(self):
        print("\n[TEST] holidays are rejected even on otherwise open days")

        service = APPOINTMENT_SERVICES[0]
        booking_date = self.next_open_date()

        with patch("website.forms.CLINIC_HOLIDAYS", {booking_date: "Clinic Maintenance Day"}):
            form = AppointmentForm(data={
    ...
    def test_reject_same_day_within_cutoff_window(self):
        print("\n[TEST] same-day bookings inside cutoff window are rejected")
        ...
        with (
            patch("website.forms.timezone.localdate", return_value=booking_date),
            patch("website.forms.timezone.now", return_value=simulated_now),
            patch("website.forms.CLINIC_HOLIDAYS", {}),
        ):
            form = AppointmentForm(data={
                "name": "Cutoff Patient",
                "phone": "09170000007",
    ...
    def test_allow_same_day_outside_cutoff_window(self):
        print("\n[TEST] same-day bookings outside cutoff window are allowed")
    ...
    def test_staff_form_allows_closed_day_and_off_hours(self):
        print("\n[TEST] staff form bypasses public schedule restrictions")
    ...
    def test_staff_form_still_blocks_slot_collision(self):
        print("\n[TEST] staff form still blocks duplicate occupied slots")
